...
BigInt::operator int() const{
    ...
    if (digits[digits.size()-1] < 0) {
        sign = -1;
        res = digits[digits.size()-1] * -1;
    } else res = digits[digits.size()-1];
...
BigInt &BigInt::operator--(){
    ...
    if (digits[0] == 0 && digits.size() == 1) throw std::out_of_range("UNDERFLOW");
...
BigInt &BigInt::operator/=(const BigInt& a){
    ...
    b.digits = a.digits;
    int sign = 1;
    if (a == zero) throw std::runtime_error("Math error: Attempted to divide by Zero\n");
